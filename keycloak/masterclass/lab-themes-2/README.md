...
7) Sucht in der `login.ftl` den `TODO`-Kommentar. Fügt dort eine FreeMarker-`<#if>`-Anweisung ein, die einen **"⚠ Demo Environment"**-Hinweis anzeigt, wenn der Realm-Name `demo-realm` ist. Schaut euch auch die anderen `<#if>`-Blöcke in der Datei an (z. B. `<#if message??>` oder `<#if realm.rememberMe>`) und versteht, wie Keycloak-Variablen in FreeMarker geprüft werden.
   ```ftl
...
8) Legt einen zweiten Realm mit dem Namen `demo-realm` an, weist ihm das `acme-theme` als Login-Theme zu und öffnet die Login-Seite. Prüft, ob der "Demo Environment"-Hinweis erscheint – und dass er bei einem Realm mit einem anderen Namen **nicht** angezeigt wird.
...
8) Create a second realm named exactly `demo-realm`, assign the `acme-theme` login theme to it and open the login page. Verify that the "Demo Environment" banner from step 7 appears – and that it does **not** appear in a realm with a different name.
