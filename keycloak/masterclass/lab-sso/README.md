...
5) Konfiguriert nun Nextcloud (http://localhost:8081) für OIDC (Hinweis: manchmal sind die Übersetzungen nicht selbstsprechend ;) ):
   ...
   * Geht zu `Verwaltung` (Administration settings) → `Social login` und richtet einen neuen Provider ein:
     ...
     * Authorize URL: `http://localhost:8080/realms/lab-realm/protocol/openid-connect/auth`
     * Token-URL: `http://keycloak:8080/realms/lab-realm/protocol/openid-connect/token`
     * Userinfo-URL: `http://keycloak:8080/realms/lab-realm/protocol/openid-connect/userinfo`
     * Logout-URL: `http://localhost:8080/realms/lab-realm/protocol/openid-connect/logout`
     * Client ID: `nextcloud`
...
8) Schaut euch die Sessions in der Admin Console für den Realm `lab-realm` an.
9) Loggt euch in der Nextcloud aus und prüft die Sessions in der Admin Console für den Realm `lab-realm` --> die SSO-Session sollte weg sein.
10) Prüft, ob ihr noch in Outline eingeloggt seid -> wir haben eine Zombie Session
11) Schaut euch die Sessions in der Admin Console für den Realm `lab-realm` an.
...
1) Start the lab via `docker compose up`. The realm `lab-realm` with two OIDC clients (`outline`, `nextcloud`) and a test user (`testuser` / `test`) will be imported automatically.
2) Verify in the Keycloak Admin Console (http://localhost:8080) that the realm `lab-realm`, both clients, and the test user exist.
3) In the `lab-realm` realm settings/Tokens -> Access Token Lifespan, set the value to 15 minutes. (so that our access token doesn't expire too quickly for this exercise)
...
8) Check the sessions in the Admin Console for the `lab-realm` realm.
9) Log out from Nextcloud and check the sessions in the Admin Console for the `lab-realm` realm --> the SSO session should be gone.
10) Check whether you are still logged in to Outline -> we have a zombie session.
11) Check the sessions in the Admin Console for the `lab-realm` realm.
